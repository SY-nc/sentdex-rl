...
for episode in range(HM_EPISODES):
    ...
    for i in range(0, SIZE):
        ...
        brick_vl.x = i*s
        brick_vl.y = 0
        enemy.append(brick_vl)

        brick_vr = Blob()
        brick_vr.x = i*s
        brick_vr.y = SIZE-1*s
        enemy.append(brick_vr)

        #horizontal-top boundary
        brick_ht = Blob()
        brick_ht.x = SIZE-1*s
        brick_ht.y = i*s
        enemy.append(brick_ht)

        brick_hb = Blob()
        brick_hb.x = 0
        brick_hb.y = i*s
        enemy.append(brick_hb)
    ...
    for i in range(200*s):
        #TODO: for each enemy, calculate obs, and fetch action: finally take the action with highest q-val 
        ...
        if show:
            ...
            img = img.resize((700, 700), resample=Image.BOX) # to remove blurry boxes
