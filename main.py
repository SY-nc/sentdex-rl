...
##from keras import models ### Added for PASS (01/06/2022)
...
#epsilon = 0.6 ### Added for PASS (01/06/2022)
#epsilon = 0.0
...
class BlobEnv:
    ...
    def step(self, action):
        ...
        if reward == self.FOOD_REWARD or self.episode_step >= 200:
...
class DQNAgent:
    ...
    ## Two models: for stability and consistency to learn something because initially so much randomness
        ...
        self.model = models.load_model("models/6.1.3-400-ep-done.model")
    ...
    def create_model(self):
        ...
        #model.compile(loss= "mean_squared_error" , optimizer="adam", metrics=["mean_squared_error"]) ## From Internet
        return model
    ...
    def test(self, terminal_state, step):
        ...
        # actual_val = self.get_qs(np.array(X))
        # print("X: " + str(X))
        # print("y: " + str(y))
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'): # ascii for windows fellows
    ...
    while not done:
        ...
        #no exploration in testing model
        action = np.argmax(agent.get_qs(current_state))
        ...
        agent.test(done, step)
        # with open("logs.txt", 'a') as out:
        #     out .write("step count = " + str(step) + ", episode count = " + str(episode) + '\n')
    ...
    if not episode % AGGREGATE_STATS_EVERY or episode == 1:
        ...
        print("reward in current ep:" + str(average_reward))
